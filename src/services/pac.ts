...
export async function recordPacPayment(
  uid: string,
  data: Omit<PacPayment, 'id' | 'createdAt' | 'updatedAt' | 'quantityPurchased'>
  ...
  try {
    ...
    const docRef = await addDoc(collection(db, COLLECTION(uid)), payment)
    ...
    await logAuditEvent(uid, {
      ...
      entityId: docRef.id,
      metadata: { investmentId: data.investmentId, importo: data.importo },
    })
...
export async function updatePacPayment(
  ...
  try {
    ...
    if (data.importo !== undefined || data.priceAtPayment !== undefined) {
      ...
      if (!snap.exists()) {
        return { success: false, error: 'Versamento non trovato' }
      }
 ...
 */
export async function getAllPacPayments(uid: string): Promise<ApiResult<PacPayment[]>> {
  try {
    const db = getFirestore()
...
export async function getPacAnalytics(
  ...
  try {
    ...
    const mediaRitorno =
      summaries.reduce((s, p) => s + p.pnlPercent, 0) / summaries.length
